...
vars = c("ELEFP", "ELEP", "HINCP", "GASFP", "GASP", "WATFP", 
         "WATP", "TEN", "RELSHIPP", "RACBLK")
...
knoxville_pums <- knoxville_pums %>%
    mutate(tenure = case_when(TEN_label == 'Owned with mortgage or loan (include home equity loans)' ~ 'Owner Occupied',
...
knoxville_pums <- knoxville_pums %>% 
    mutate(kub_energy = (case_when(GASFP_label == 'Valid monthly gas cost in GASP' ~ GASP,
...
knoxville_pums_filtered <- knoxville_pums %>% filter(kub_burden >=0 & kub_burden <=1)
    
knoxville_pums_filtered %>% 
    srvyr::survey_count(RACBLK_label, kub_burden > 0.1)

knoxville_pums_filtered %>%
    srvyr::survey_count(RACBLK_label, tenure, kub_burden > 0.1)

model <- survey::svyglm(I(kub_burden > 0.1) ~ black + tenure, design=knoxville_pums_filtered)
